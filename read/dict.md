...
Redis 实现使用的C语言并没有内置 dict 这种数据类型，所以 Redis 实现了自己的 dict。dict 在 Redis 中有广泛的应用，Redis 数据库的底层就是使用字典来实现的，也就是说对数据库的增，删，改，查是建立在字典之上的。同时，字典也是`hash`和`sorted set`等数据结构的底层实现之一。

## dict 的实现

在 Redis 中，dict 的代码存放在`src/dict.c`和`src/dict.h`两个文件中。在`src/dict.h`中定义了 dict 所用到的结构体，一共有如下四个:
...
根据前面的内容所知，向dict中添加键值对时，dict根据键计算出来的索引值将键值对存储到哈希表中。由于哈希表的大小是有限的，所以可能会出现两个键值对的索引相同的情况，这种情况也称作键冲突。
...
其中k1和k2计算出来的索引值是相同的，所以他们被存放到哈希表的同一个索引下，然后通过单链表的形式组合起来。

在 `src/dict.c`的`dictEntry *dictAddRaw(dict *d, void *key)`函数中，我们可以向单链表中添加元素的过程
...
当 dict 的负载因子过大或过小的时候，Redis 需要为这个 dict 扩展或者收缩哈希表的大小，这个过程叫做 rehash。上文中我们提到每个 dict 都有两个哈希表, ht[0]和ht[1]，在这里它们就发挥作用了。rehash 的具体操作如下:
...
3. 在增删改查的过程中，检查 dict.rehashidx 是不是不等于 -1，如果是，那么就执行 _dictRehashStep 操作，_dictRehashStep 的操作如下:
	...
	+ dictRehash 操作就是从 ht[0] 中取出值，重新计算哈希值，然后放到 ht[1] 中，并将ht[0]的 used 减少。将 dict.rehashidx 的索引加。由于在这个过程中重新计算了 key 的哈希值，所以这个过程称为 rehash
...
假设我们在一个正在进行 rehash 操作的字典上 创立了一个安全迭代器，首先执行了`dictNext()`，获取了ht[0]中的一个节点node1。接着执行`dictAdd()`，添加了一个节点。如果`dictAdd()`函数能够执行 rehash 的话，node1 节点可能被从ht[0]复制到了ht[1]，从而导致安全迭代器获取了节点node1两次。

所以当字典中存在安全迭代器的时候，是不能进行 rehash 操作的。

而非安全迭代器，Redis 规定在非安全迭代器的声明周期内只能执行 dictNext() 操作，不能执行其他操作，从而避免了 rehash 操作。

同时 Redis 在创建非安全迭代器的时候也获取 dict 的指纹，在释放非安全迭代器的时候重新获取指纹，检查两次指纹是否相同。如果不同，证明 dict 被修改过，Redis 就会抛出异常。